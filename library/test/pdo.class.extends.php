...
{
    protected $database_types = array('sqlite2', 'sqlite3', 'sqlsrv', 'mssql', 'mysql', 'pg', 'ibm', 'dblib', 'odbc', 'oracle', 'ifmx', 'fbd');
     ...
     */
    public function load($database_type, $host, $database, $user, $password, $port)
    {
        ...
        if (in_array($this->database_type, $this->database_types)) {
            try {
                switch ($this->database_type) {
                    case 'mssql':
                        $this->con = parent::__construct('mssql:host='.$this->host.';dbname='.$this->database, $this->user, $this->password);
                        break;
                    case 'sqlsrv':
                        $this->con = parent::__construct('sqlsrv:server='.$this->host.';database='.$this->database, $this->user, $this->password);
                        break;
                    case 'ibm': //default port = ?
                        $this->con = parent::__construct('ibm:DRIVER={IBM DB2 ODBC DRIVER};DATABASE='.$this->database.'; HOSTNAME='.$this->host.';PORT='.$this->port.';PROTOCOL=TCPIP;', $this->user, $this->password);
                        break;
                    case 'dblib': //default port = 10060
                        $this->con = parent::__construct('dblib:host='.$this->host.':'.$this->port.';dbname='.$this->database, $this->user, $this->password);
                        break;
                    case 'odbc':
                        $this->con = parent::__construct("odbc:Driver={Microsoft Access Driver (*.mdb)};Dbq=C:\accounts.mdb;Uid=".$this->user);
                        break;
                    case 'oracle':
                        $this->con = parent::__construct('OCI:dbname='.$this->database.';charset=UTF-8', $this->user, $this->password);
                        break;
                    case 'ifmx':
                        $this->con = parent::__construct('informix:DSN=InformixDB', $this->user, $this->password);
                        break;
                    case 'fbd':
                        $this->con = parent::__construct('firebird:dbname='.$this->host.':'.$this->database, $this->user, $this->password);
                        break;
                    case 'mysql':
                        $this->con = (is_numeric($this->port)) ? parent::__construct('mysql:host='.$this->host.';port='.$this->port.';dbname='.$this->database, $this->user, $this->password) : parent::__construct('mysql:host='.$this->host.';dbname='.$this->database, $this->user, $this->password);
                        break;
                    case 'sqlite2': //ej: "sqlite:/path/to/database.sdb"
                        $this->con = parent::__construct('sqlite:'.$this->host);
                        break;
                    case 'sqlite3':
                        $this->con = parent::__construct('sqlite::memory');
                        break;
                    case 'pg':
                        $this->con = (is_numeric($this->port)) ? parent::__construct('pgsql:dbname='.$this->database.';port='.$this->port.';host='.$this->host, $this->user, $this->password) : parent::__construct('pgsql:dbname='.$this->database.';host='.$this->host, $this->user, $this->password);
                        break;
                    default:
                        $this->con = null;
                        break;
                    ...
                    case 'update':
                        ...
                        if ((isset($arguments[0]) && gettype($arguments[0]) !== 'array') || gettype($arguments) == 'array') {
                            ...
                            for ($i = 0; $i < $total_elements; ++$i) {
                                if (gettype($arguments[$i] == 'array')) {
                                    $sth->execute($arguments[$i]);
                                }
                            }
                        }
